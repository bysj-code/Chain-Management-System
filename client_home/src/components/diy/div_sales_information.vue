<template>
	<view class="diy_details diy_div_sales_information">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','sales_date')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>销售日期:</span>
						</view>
							<view class="diy_field diy_date">
							<text>
								{{ $toTime(obj["sales_date"],"yyyy-MM-dd") }}
							</text>
						</view>
						</view>
					...
					<view v-if="$check_field('get','sales_amount')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>销售金额:</span>
						</view>
							<view class="diy_field diy_text">
					...
					<view v-if="$check_field('get','sales_remarks')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>销售备注:</span>
						</view>
			...
			<view class="container">
				<view class="row">
					<view class="col">
						<button type="button" class="diy_btn" @click="to_form('/pages/order_transportation/edit')" v-if="$check_action('/order_transportation/edit','add') || $check_action('/order_transportation/edit','set')" >运输</button>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/sales_information/get_obj", {
					sales_information_id:this.query.sales_information_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["sales_information_id"] !==0) {
