						...
						<view class="register_view">
							<uni-forms class="form_register" :rules="rules" :value="form" ref="form"
								...
								<uni-forms-item class="user_group" name="user_group" label="身份">
									...
									<!-- 人事部 -->
								<form_ministry_of_personnel v-if="form.user_group === '人事部'" @change="handleSubFormChange" :form="form_sub" ></form_ministry_of_personnel>
										<!-- 财务部 -->
								<form_finance_department v-if="form.user_group === '财务部'" @change="handleSubFormChange" :form="form_sub" ></form_finance_department>
										<!-- 销售部 -->
								<form_sales_department v-if="form.user_group === '销售部'" @change="handleSubFormChange" :form="form_sub" ></form_sales_department>
										<!-- 采购部 -->
								<form_procurement_department v-if="form.user_group === '采购部'" @change="handleSubFormChange" :form="form_sub" ></form_procurement_department>
										<!-- 服务部 -->
								<form_service_department v-if="form.user_group === '服务部'" @change="handleSubFormChange" :form="form_sub" ></form_service_department>
...
<script>
	import mixin from "@/libs/mixins/page.js"
		/* 人事部 */
	import form_ministry_of_personnel from "@/components/diy/form_ministry_of_personnel";
			/* 财务部 */
	import form_finance_department from "@/components/diy/form_finance_department";
			/* 销售部 */
	import form_sales_department from "@/components/diy/form_sales_department";
			/* 采购部 */
	import form_procurement_department from "@/components/diy/form_procurement_department";
			/* 服务部 */
	import form_service_department from "@/components/diy/form_service_department";
									
	export default {
		components: {
				/* 人事部 */
			form_ministry_of_personnel,
					/* 财务部 */
			form_finance_department,
					/* 销售部 */
			form_sales_department,
					/* 采购部 */
			form_procurement_department,
					/* 服务部 */
			form_service_department,
											},
		mixins: [mixin],
		data() {
			return {
			...
			async submit_check(param){
				...
				if(!ret && user_group){
						...
						var form_sub = Object.assign({} ,this.form_sub);
							...
							res = await this.$get("~/api/ministry_of_personnel/count?", {"employee_id": form_sub["employee_id"]});
							if(res.result){
								ret = "员工工号已存在!";
							}
						}
						...
						var form_sub = Object.assign({} ,this.form_sub);
							...
							res = await this.$get("~/api/finance_department/count?", {"employee_id": form_sub["employee_id"]});
							if(res.result){
								ret = "员工工号已存在!";
							}
						}
						...
						var form_sub = Object.assign({} ,this.form_sub);
							...
							res = await this.$get("~/api/sales_department/count?", {"employee_id": form_sub["employee_id"]});
							if(res.result){
								ret = "员工工号已存在!";
							}
						}
						...
						var form_sub = Object.assign({} ,this.form_sub);
							...
							res = await this.$get("~/api/procurement_department/count?", {"employee_id": form_sub["employee_id"]});
							if(res.result){
								ret = "员工工号已存在!";
							}
						}
						...
						var form_sub = Object.assign({} ,this.form_sub);
							...
							res = await this.$get("~/api/service_department/count?", {"employee_id": form_sub["employee_id"]});
							if(res.result){
								ret = "员工工号已存在!";
							}
						}
