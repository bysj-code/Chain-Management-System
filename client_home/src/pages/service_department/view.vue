          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','employee_id') || ($check_field('add','employee_id') || $check_field('set','employee_id'))" label="员工工号" name="employee_id">
                            <uni-easyinput type="text" v-model="form['employee_id']" v-if="user_group === '管理员' || (form['service_department_id'] && $check_field('set','employee_id')) || (!form['service_department_id'] && $check_field('add','employee_id'))" :disabled="disabledObj['employee_id_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','employee_name') || ($check_field('add','employee_name') || $check_field('set','employee_name'))" label="员工姓名" name="employee_name">
                            <uni-easyinput type="text" v-model="form['employee_name']" v-if="user_group === '管理员' || (form['service_department_id'] && $check_field('set','employee_name')) || (!form['service_department_id'] && $check_field('add','employee_name'))" :disabled="disabledObj['employee_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','employee_gender') || ($check_field('add','employee_gender') || $check_field('set','employee_gender'))" label="员工性别" name="employee_gender">
                  ...
                  v-if="user_group === '管理员' || (form['service_department_id'] && $check_field('set','employee_gender')) || (!form['service_department_id'] && $check_field('add','employee_gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "service_department",
      is_password: true,
      field: "service_department_id",
      url_add: "~/api/service_department/add?",
      url_set: "~/api/service_department/set?",
      url_get_obj: "~/api/service_department/get_obj?",
      url_upload: "~/api/service_department/upload?",

      query: {
        "service_department_id": 0,
      },
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/service_department/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["service_department_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?service_department_id=" + form_sub["service_department_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/service_department/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/service_department/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/service_department/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/service_department/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/service_department/view','get');
