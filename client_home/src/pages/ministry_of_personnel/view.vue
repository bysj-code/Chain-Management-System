          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','employee_id') || ($check_field('add','employee_id') || $check_field('set','employee_id'))" label="员工工号" name="employee_id">
                            <uni-easyinput type="text" v-model="form['employee_id']" v-if="user_group === '管理员' || (form['ministry_of_personnel_id'] && $check_field('set','employee_id')) || (!form['ministry_of_personnel_id'] && $check_field('add','employee_id'))" :disabled="disabledObj['employee_id_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','employee_name') || ($check_field('add','employee_name') || $check_field('set','employee_name'))" label="员工姓名" name="employee_name">
                            <uni-easyinput type="text" v-model="form['employee_name']" v-if="user_group === '管理员' || (form['ministry_of_personnel_id'] && $check_field('set','employee_name')) || (!form['ministry_of_personnel_id'] && $check_field('add','employee_name'))" :disabled="disabledObj['employee_name_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','employee_name')">
                  {{ form['employee_name'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','employee_gender') || ($check_field('add','employee_gender') || $check_field('set','employee_gender'))" label="员工性别" name="employee_gender">
                    <uni-data-select
                  v-model="form.employee_gender"
                  :localdata="list_employee_gender"
                  :clear="!disabledObj['employee_gender_isDisabled']"
                  :disabled="disabledObj['employee_gender_isDisabled']"
                  v-if="user_group === '管理员' || (form['ministry_of_personnel_id'] && $check_field('set','employee_gender')) || (!form['ministry_of_personnel_id'] && $check_field('add','employee_gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "ministry_of_personnel",
      is_password: true,
      field: "ministry_of_personnel_id",
      url_add: "~/api/ministry_of_personnel/add?",
      url_set: "~/api/ministry_of_personnel/set?",
      url_get_obj: "~/api/ministry_of_personnel/get_obj?",
      url_upload: "~/api/ministry_of_personnel/upload?",

      query: {
        "ministry_of_personnel_id": 0,
      },
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/ministry_of_personnel/upload?'),
    ...
    async get_list_employee_gender() {
          ['男','女'].map((o) => this.list_employee_gender.push({value:o,text:o}));
            },
    ...
    submit_sub(form_sub){
      if(form_sub["ministry_of_personnel_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?ministry_of_personnel_id=" + form_sub["ministry_of_personnel_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/ministry_of_personnel/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/ministry_of_personnel/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/ministry_of_personnel/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/ministry_of_personnel/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/ministry_of_personnel/view','get');
