<template>
  <view class="page_procurement_department diy_table diy_table_tml" id="procurement_department_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="员工工号" name="employee_id">
                      <uni-easyinput type="text" v-model="query.employee_id" placeholder="员工工号" />
                    </uni-forms-item>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/procurement_department/view?" v-show="user_group == '管理员' || $check_action('/procurement_department/table','add') || $check_action('/procurement_department/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="1 && $check_field('get','employee_id')" class="col">
						<view class="view">
                            ...
                            <span>员工工号</span>
								...
								<text>
									{{ o["employee_id"] }}
													...
													<view v-if="1 && $check_field('get','employee_name')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>员工姓名</span>
                          </view>
								...
								<text>
									{{ o["employee_name"] }}
													...
													<view v-if="1 && $check_field('get','employee_gender')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>员工性别</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/procurement_department/table','del') || $check_action('/procurement_department/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/procurement_department/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/procurement_department/table','set') || $check_action('/procurement_department/view','set') || $check_action('/procurement_department/view','get')" class="btn btn-primary">详情</navigator>
	        					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/procurement_department/get_list?like=0",
				url_del: "~/api/procurement_department/del?",
				...
				list: [],
                                    // 员工性别列表
                list_employee_gender: [{value:"",text:"全部"}],
        			}
		},
		methods: {
          search() {
